# 2026 수능 LLM 풀이 기록

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Python 3.8+](https://img.shields.io/badge/python-3.8+-blue.svg)](https://www.python.org/downloads/)

## 개요
다양한 LLM 모델의 2026학년도 수능 문제 풀이 결과를 기록하고 통계를 정리한 데이터입니다.

## 주요 과목 종합 성적

![주요 과목-모델별 총점 비교](images/subject_model_comparison.png)

> 📊 **평가 과목**: 국어, 수학, 영어, 한국사 (총 350점 만점)
>
> 📝 **계산 방식**:
> - 국어: 공통 + 선택과목(화법과 작문, 언어와 매체) 평균
> - 수학: 공통 + 선택과목(확률과 통계, 미적분, 기하) 평균
> - 영어, 한국사: 전체 점수
>
> 💡 각 과목별 상세 성적은 아래에서 확인할 수 있습니다.

## 테스트 환경 및 주의사항

**중요: 본 테스트는 API를 통한 최적화된 환경에서 수행되었습니다.**

### 테스트 환경
- **실행 방식**: 각 모델의 공식 API를 통해 진행
- **추론 설정**:
  - 추론 예산(reasoning budget): **최대값으로 설정**
  - 최대 출력 토큰(max output tokens): **최대값으로 설정**
- **시스템 프롬프트**: 별도의 시스템 프롬프트 제공하지 않음
- **외부 도구**: 검색, 계산기 등 외부 도구를 **제공하지 않음**
- **테스트 방법**: 순수하게 모델의 기본 능력만으로 문제 해결

### 일반 사용자 환경과의 차이
본 테스트 결과는 일반적인 공식 홈페이지/앱 사용 환경과 다를 수 있습니다:

1. **추론 예산 제한**: 일반 사용자 환경에서는 추론 예산이 제한되어 있어, 본 테스트보다 낮은 성능을 보일 수 있습니다.
2. **출력 토큰 제한**: 일반 사용자 환경에서는 최대 출력 토큰이 제한되어 있어, 긴 추론 과정이 필요한 문제에서 답변이 잘릴 수 있습니다.
3. **도구 미사용**: 일반 사용자 환경에서는 필요에 따라 웹 페이지 검색 등을 수행할 수 있으나, 본 테스트는 순수 모델의 지식만으로 답변했습니다.

따라서 **공식 홈페이지나 앱에서와 성능에 차이가 있을 수 있습니다.**

### 평가 방식
- 각 LLM 모델에 문제 텍스트를 제시하고 답안 선택
- 단답형 문항의 경우 정확한 숫자 입력 요구
- 모델이 제시한 최종 답안을 정답과 비교하여 채점

### 문제 제공 방식
- **텍스트 추출**: PDF 파일을 OCR 등을 이용하여 텍스트로 변환하여 제공
- **이미지 처리**: 문제에 포함된 이미지(그래프, 도표, 그림 등)만 별도로 캡처하여 제공
- **원본 유지**: PDF 파일 자체나 전체 페이지 캡처는 제공하지 않음
- 이는 모델이 순수하게 텍스트 이해 능력과 시각 자료 해석 능력을 활용하도록 하기 위함

## 테스트 모델
- **OpenAI GPT 시리즈**
  - GPT-5.1
  - GPT-5.1 Chat
  - GPT-5 mini
  - GPT-5 nano

  ※ ChatGPT 공식 서비스 기준, 평상시 응답 모델은 GPT-5.1 Chat(Instant), '오래 생각하기' 활성화 시 응답하는 모델은 GPT-5.1입니다.

- **Google Gemini 시리즈**
  - Gemini 2.5 Pro
  - Gemini 2.5 Flash (2025-09 Preview)
  - Gemini 2.5 Flash Lite (2025-09 Preview)

## 국어 영역

### 모델별 종합 성적 (100점 만점)

#### 화법과 작문 선택 시
| 순위 | 모델 | 점수 |
|:---:|:-----|:------:|
| 1 | GPT-5.1 | 98점 |
| 1 | Gemini 2.5 Pro | 98점 |
| 3 | Gemini 2.5 Flash | 97점 |
| 4 | GPT-5 mini | 90점 |
| 5 | Gemini 2.5 Flash Lite | 85점 |
| 6 | GPT-5.1 Chat | 84점 |
| 7 | GPT-5 nano | 47점 |

#### 언어와 매체 선택 시
| 순위 | 모델 | 점수 |
|:---:|:-----|:------:|
| **1** | **Gemini 2.5 Pro** | **100점** |
| 2 | GPT-5.1 | 98점 |
| 3 | Gemini 2.5 Flash | 95점 |
| 4 | GPT-5 mini | 91점 |
| 5 | GPT-5.1 Chat | 79점 |
| 6 | Gemini 2.5 Flash Lite | 78점 |
| 7 | GPT-5 nano | 43점 |

### 영역별 상세 성적

#### 국어 - 공통 (76점 만점)
| 모델 | 점수 |
|:-----|:------:|
| **Gemini 2.5 Pro** | **76점** |
| GPT-5.1 | 74점 |
| Gemini 2.5 Flash | 73점 |
| GPT-5 mini | 69점 |
| Gemini 2.5 Flash Lite | 61점 |
| GPT-5.1 Chat | 60점 |
| GPT-5 nano | 32점 |

#### 국어 - 화법과 작문 (24점 만점)
| 모델 | 점수 |
|:-----|:------:|
| **GPT-5.1** | **24점** |
| **GPT-5.1 Chat** | **24점** |
| **Gemini 2.5 Flash** | **24점** |
| **Gemini 2.5 Flash Lite** | **24점** |
| Gemini 2.5 Pro | 22점 |
| GPT-5 mini | 21점 |
| GPT-5 nano | 15점 |

#### 국어 - 언어와 매체 (24점 만점)
| 모델 | 점수 |
|:-----|:------:|
| **Gemini 2.5 Pro** | **24점** |
| **GPT-5.1** | **24점** |
| GPT-5 mini | 22점 |
| Gemini 2.5 Flash | 22점 |
| GPT-5.1 Chat | 19점 |
| Gemini 2.5 Flash Lite | 17점 |
| GPT-5 nano | 11점 |

### 시각적 성적 비교

#### 종합 성적 비교 (화법과 작문)
![화법과 작문 성적 비교](images/국어_score_common_화작.png)

#### 종합 성적 비교 (언어와 매체)
![언어와 매체 성적 비교](images/국어_score_common_언매.png)

#### 영역별 점수 분포 (화법과 작문)
![화법과 작문 영역별 분포](images/국어_breakdown_화작.png)

#### 영역별 점수 분포 (언어와 매체)
![언어와 매체 영역별 분포](images/국어_breakdown_언매.png)

---

## 수학 영역

### 모델별 종합 성적 (100점 만점)

#### 확률과 통계 선택 시
| 순위 | 모델 | 점수 |
|:---:|:-----|:------:|
| **1** | **GPT-5.1** | **100점** |
| 2 | GPT-5 mini | 96점 |
| 2 | GPT-5 nano | 96점 |
| 2 | Gemini 2.5 Pro | 96점 |
| 5 | GPT-5.1 Chat | 92점 |
| 5 | Gemini 2.5 Flash | 92점 |
| 7 | Gemini 2.5 Flash Lite | 84점 |

#### 미적분 선택 시
| 순위 | 모델 | 점수 |
|:---:|:-----|:------:|
| **1** | **GPT-5.1** | **100점** |
| 2 | GPT-5 mini | 96점 |
| 2 | Gemini 2.5 Flash | 96점 |
| 4 | GPT-5 nano | 92점 |
| 4 | Gemini 2.5 Pro | 92점 |
| 6 | Gemini 2.5 Flash Lite | 88점 |
| 7 | GPT-5.1 Chat | 84점 |

#### 기하 선택 시
| 순위 | 모델 | 점수 |
|:---:|:-----|:------:|
| **1** | **GPT-5.1** | **100점** |
| 2 | GPT-5 nano | 96점 |
| 2 | Gemini 2.5 Pro | 96점 |
| 2 | Gemini 2.5 Flash | 96점 |
| 5 | GPT-5 mini | 92점 |
| 5 | Gemini 2.5 Flash Lite | 92점 |
| 7 | GPT-5.1 Chat | 88점 |

### 영역별 상세 성적

#### 수학 - 공통 (74점 만점)
| 모델 | 점수 |
|:-----|:------:|
| **GPT-5.1** | **74점** |
| GPT-5 mini | 70점 |
| GPT-5 nano | 70점 |
| Gemini 2.5 Pro | 70점 |
| Gemini 2.5 Flash | 70점 |
| GPT-5.1 Chat | 66점 |
| Gemini 2.5 Flash Lite | 66점 |

#### 수학 - 확률과 통계 (26점 만점)
| 모델 | 점수 |
|:-----|:------:|
| **GPT-5.1** | **26점** |
| **GPT-5.1 Chat** | **26점** |
| **GPT-5 mini** | **26점** |
| **GPT-5 nano** | **26점** |
| **Gemini 2.5 Pro** | **26점** |
| Gemini 2.5 Flash | 22점 |
| Gemini 2.5 Flash Lite | 18점 |

#### 수학 - 미적분 (26점 만점)
| 모델 | 점수 |
|:-----|:------:|
| **GPT-5.1** | **26점** |
| **GPT-5 mini** | **26점** |
| **Gemini 2.5 Flash** | **26점** |
| GPT-5 nano | 22점 |
| Gemini 2.5 Pro | 22점 |
| Gemini 2.5 Flash Lite | 22점 |
| GPT-5.1 Chat | 18점 |

#### 수학 - 기하 (26점 만점)
| 모델 | 점수 |
|:-----|:------:|
| **GPT-5.1** | **26점** |
| **GPT-5 nano** | **26점** |
| **Gemini 2.5 Pro** | **26점** |
| **Gemini 2.5 Flash** | **26점** |
| **Gemini 2.5 Flash Lite** | **26점** |
| GPT-5.1 Chat | 22점 |
| GPT-5 mini | 22점 |

### 시각적 성적 비교

#### 종합 성적 비교 (확률과 통계)
![확률과 통계 성적 비교](images/수학_score_common_확통.png)

#### 종합 성적 비교 (미적분)
![미적분 성적 비교](images/수학_score_common_미적.png)

#### 종합 성적 비교 (기하)
![기하 성적 비교](images/수학_score_common_geometry.png)

#### 영역별 점수 분포 (확률과 통계)
![확률과 통계 영역별 분포](images/수학_breakdown_확통.png)

#### 영역별 점수 분포 (미적분)
![미적분 영역별 분포](images/수학_breakdown_미적.png)

#### 영역별 점수 분포 (기하)
![기하 영역별 분포](images/수학_breakdown_geometry.png)

---

## 영어 영역

### 모델별 성적 (100점 만점)

| 순위 | 모델 | 점수 |
|:---:|:-----|:------:|
| **1** | **GPT-5.1** | **100점** |
| **1** | **GPT-5 mini** | **100점** |
| 3 | GPT-5.1 Chat | 97점 |
| 3 | Gemini 2.5 Pro | 97점 |
| 5 | Gemini 2.5 Flash | 95점 |
| 6 | GPT-5 nano | 93점 |
| 7 | Gemini 2.5 Flash Lite | 92점 |

### 시각적 성적 비교

![영어 성적 비교](images/영어_score_전체.png)

---

## 한국사 영역

### 모델별 성적 (50점 만점)

| 순위 | 모델 | 점수 |
|:---:|:-----|:------:|
| **1** | **GPT-5.1** | **50점** |
| **1** | **GPT-5.1 Chat** | **50점** |
| **1** | **GPT-5 mini** | **50점** |
| **1** | **Gemini 2.5 Pro** | **50점** |
| **1** | **Gemini 2.5 Flash** | **50점** |
| **1** | **Gemini 2.5 Flash Lite** | **50점** |
| 7 | GPT-5 nano | 37점 |

### 시각적 성적 비교

![한국사 성적 비교](images/한국사_score_전체.png)

---

## 차트 생성 스크립트

프로젝트에는 자동화된 차트 생성 스크립트가 포함되어 있습니다.

### 특징
- 완전 자동화: 엑셀 파일의 모든 시트 자동 감지
- 확장 가능: 새로운 과목/모델 추가 시 코드 수정 불필요
- 유연한 옵션: 특정 과목만, 또는 특정 차트 유형만 생성 가능
- 전과목 합산: 모든 과목의 점수를 합산한 종합 비교 차트 생성

### 사용법

```bash
# 모든 차트 생성 (과목별 차트 + 전과목 합산 차트)
python generate_charts.py

# 전과목 합산 차트만 생성
python generate_charts.py --overall

# 과목별 차트만 생성 (전과목 합산 제외)
python generate_charts.py --no-overall

# 사용 가능한 과목 확인
python generate_charts.py --list

# 특정 과목만 생성
python generate_charts.py --subjects 국어
python generate_charts.py --subjects 국어 수학

# 종합 차트만 생성
python generate_charts.py --mode summary

# 영역별 분포 차트만 생성
python generate_charts.py --mode breakdown
```

### 자동 감지 기능
- **과목 감지**: `과목명-영역명` 형식의 시트 자동 인식
- **모델 감지**: '문항 번호'와 '정답'을 제외한 모든 컬럼을 모델로 인식
- **점수 감지**: '총점', '총합', '점수' 행에서 자동 추출

### 확장 방법
새로운 과목이나 모델을 추가하려면:
1. 엑셀 파일에 `과목명-공통`, `과목명-선택과목` 형식으로 시트 추가
2. 모델 컬럼 추가
3. `python generate_charts.py` 실행

코드 수정 없이 자동으로 차트가 생성됩니다.

---

## 평가 대상 상세

### 국어 영역
- **평가 대상**: 2026학년도 대학수학능력시험 국어 영역
  - 공통 문항: 34문항 (76점)
  - 선택 과목: 화법과 작문 (11문항, 24점) / 언어와 매체 (11문항, 24점)
- **총점**: 100점 만점 (공통 76점 + 선택과목 24점)

### 수학 영역
- **평가 대상**: 2026학년도 대학수학능력시험 수학 영역
  - 공통 문항: 22문항 (74점)
  - 선택 과목: 확률과 통계 (8문항, 26점) / 미적분 (8문항, 26점) / 기하 (8문항, 26점)
- **총점**: 100점 만점 (공통 74점 + 선택과목 26점)

### 영어 영역
- **평가 대상**: 2026학년도 대학수학능력시험 영어 영역
  - 전체 문항: 45문항
- **총점**: 100점 만점

### 한국사 영역
- **평가 대상**: 2026학년도 대학수학능력시험 한국사 영역
  - 전체 문항: 20문항
- **총점**: 50점 만점 (필수 영역)

## 평가 방법 상세

OCR을 이용해 우선 문제를 텍스트로 추출한 후, 잘못 옮겨진 부분은 수작업으로 수정했습니다.

모든 문제는 필요한 최소한의 정보만을 프롬프트로 각각 입력하였습니다.
지문 1개에 연계 문제가 있는 구조라면 지문+해당 문제 개별 출제와 같은 방식입니다.

수식은 LaTeX로 변환하여 입력해 식 자체를 잘못 이해하는 것을 방지했습니다. 이미지에서 텍스트를 인식하는 것 또한 성능 지표이긴 하지만, 문제를 잘못 인식해 틀리는 것은 본 프로젝트의 방향성에 맞지 않는다고 판단하였습니다.

오류로 인해 응답을 아예 받지 못한 경우에만 재시도 하였으며, 다른 형태의 잘못된 응답은 모두 오답 처리 했습니다.

### 입력 프롬프트 예시

- 지문이 있는 경우 (국어 홀수형 3번 문항):
````
[1～3] 다음 글을 읽고 물음에 답하시오.

```
 글을 읽고 그 의미를 이해하는 독해에는 글의 유형이나 독서 흥미 등의 다양한 요소가 영향을 미칠 수 있다. ...제공했다는 의의가 있다.
```

3. 윗글을 바탕으로 <보기>를 이해한 내용으로 적절하지 않은 것은? [3점]
```
<보기>
단순 관점을 지지하는 연구자 갑은 학생 A, B의 독해 능력을 분석하기 위한 활동을 진행하였다. ...
○ 소리 내어 단어 읽기: 학생 A는 활동 자료에 있는 단어를 빠르고 정확하게 ...
○ 중심 내용 파악하기: 학생 A는 활동 자료를 글로 읽을 때와 말로 들을 때 모두 ...
```

① 갑은 학생 A가 해독은 발달되었지만, ...
② 갑은 학생 A가 글자와 글자 소리에 대한 학습을 통해 ...
③ ...
④ ...
⑤ ...
````

- 수식이 있는 경우 (수학 홀수형 15번 문항):
````
15. 함수 $f(x)$가
$f(x) = \begin{cases} -x^2 & (x < 0) \\ x^2-x & (x \ge 0) \end{cases}$
이고, 양수 a에 대하여 함수 $g(x)$를 ... $k+h(3)$의 값은? [4점]

① $\frac{9}{2}$
② $\frac{11}{2}$
③ ...
④ ...
⑤ ...
````

- 이미지가 있는 경우 (한국사 홀수형 2번 문항):
````
2. 밑줄 친 '이 나라'에 대한 설명으로 옳은 것은?
```
[이미지: 02_01 (실제로 이미지를 첨부함)]
> **寶曆(보력) 慶雲(경운)**
유물은 고구려를 계승한 *이 나라* 제3대 왕 문왕의 배우자인 ... 고구려 후기 양식을 이어받은 것이다.
```

① 골품제를 실시하였다.
② 평양으로 천도하였다.
③ ...
④ ...
⑤ ...
````

---

## 라이선스 및 저작권

### 프로젝트 라이선스
본 프로젝트는 MIT 라이선스 하에 배포됩니다. 자세한 내용은 [LICENSE](LICENSE) 파일을 참조하세요.

### 수능 문제 저작권
- 2026학년도 대학수학능력시험 문제는 **한국교육과정평가원**의 저작물입니다.
- 본 저장소는 문제 원본을 포함하지 않으며, 오직 **모델의 답변 결과 및 분석 데이터**만 포함합니다.
- 엑셀 파일은 문항 번호와 정답만 포함하며, 전체 문제 텍스트는 저작권 문제로 제공하지 않습니다.
- 수능 문제를 활용하시려면 한국교육과정평가원의 공식 자료를 이용하시기 바랍니다.